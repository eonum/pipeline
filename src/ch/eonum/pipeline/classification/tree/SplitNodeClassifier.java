...
public class SplitNodeClassifier<E extends Instance> extends SplitNode<E> {
	...
	public SplitNodeClassifier(DecisionTree<E> parent, int depth, DataSet<E> trainSet, Features classes) {
	...
	protected void pickSplitFeature(int minSize, Features fs) {
		...
		for(int f = 0; f < fs.size(); f++){
			...
			for(Double sv : splits){
				...
				for(String className : classes.asSet()){
					double pl = geNum == 0 ? 0.0 : geClassDistribution.get(className)/(double)geNum;
					double pr = ltNum == 0 ? 0.0 : ltClassDistribution.get(className)/(double)ltNum;
					sum += Math.abs(pl - pr);
				}
				twoing *= sum;
		...
		right = new SplitNodeClassifier<E>(parent, depth + 1, rightTrain, classes);
	...
	public String toString(){
		String tree = "{ 'label' : " + label + ", 'num' : " + trainSet.size();
